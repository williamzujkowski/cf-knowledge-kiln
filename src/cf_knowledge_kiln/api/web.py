...
async def search_partial(
    ...
    filters = _filters_from_form(status)
...
def _filters_from_form(status: list[str] | None) -> RetrievalFilters:
    ...
    """
    valid = {"active", "approved", "draft", "deprecated", "archived", "superseded"}
    selected = [s for s in (status or _DEFAULT_STATUSES) if s in valid]
