...
class HttpConnector:
    ...
    SSRF guard fires twice per URL: once before the request (host
    allowlist + DNS resolution → IP-range check) and once after each
    redirect (the resolved IP of every hop must still be public).
    Responses larger than ``caps.max_file_bytes`` are refused via the
    streamed reader so we don't materialize the body in memory.
    """
    ...
    def fetch(self, source: HttpSource) -> FetchResult:
        # Lazy import — httpx is a core dep but loading it eagerly
        # adds startup time and is unnecessary for non-HTTP sources.
        import httpx
        ...
        with httpx.Client(
            ...
            follow_redirects=False,  # we manually re-guard each hop
    ...
    def _fetch_one(
        self,
        client: object,  # httpx.Client; opaque to avoid eager import in type
        url: str,
        source: HttpSource,
        ...
        try:
            ...
            # Manually follow redirects so we can re-guard each hop.
            for _ in range(5):
                ...
                if response.is_redirect:
                    ...
                    current = httpx.URL(loc) if "://" in loc else current.copy_with(path=loc)
                    continue
                ...
                # Materialize as a path-like string the rest of the
                # pipeline can use. Strip the scheme so it's
                # filesystem-friendly.
                rel = f"{current.host}{current.path or '/'}"
                fetched = FetchedFile(
                    path=rel,
                    content=body,
                    size_bytes=len(body),
...
def _guard_url(url: object, source: HttpSource) -> None:  # url is httpx.URL
