...
class TestFeedbackAckAnnouncement:
    def test_dispatcher_matches_data_feedback_ack(self, kiln_app_source: str) -> None:
        assert "data-feedback-ack" in kiln_app_source

    def test_dispatcher_announces_with_feedback_recorded_prefix(self, kiln_app_source: str) -> None:
        assert "Feedback recorded: " in kiln_app_source

    def test_dispatcher_normalizes_underscores_in_signal(self, kiln_app_source: str) -> None:
        """Underscore-to-space normalization for AT readability."""
        assert "/_/g" in kiln_app_source


class TestPreviewAnnouncement:
    def test_dispatcher_reads_preview_title_selector(self, kiln_app_source: str) -> None:
    ...
    def test_dispatcher_reads_preview_missing_selector(self, kiln_app_source: str) -> None:
        assert ".preview-missing" in kiln_app_source

    def test_dispatcher_announces_preview_loaded(self, kiln_app_source: str) -> None:
        assert "Preview loaded: " in kiln_app_source

    def test_dispatcher_announces_preview_unavailable(self, kiln_app_source: str) -> None:
